...
public class SquamorphTeethLayer extends RenderLayer<SquamorphEntity, SquamorphModel> {


    public SquamorphTeethLayer(SquamorphRenderer pRenderer) {
    ...
    public void render(PoseStack poseStack, MultiBufferSource buffer, int packedLightIn, SquamorphEntity entity, float v, float v1, float v2, float v3, float v4, float v5) {
        ...
        if (entity.getMouth().hasTeeth()) {


            renderColoredCutoutModel(getParentModel(), AllTextures.getTeethTex(entity.getMouthType()), poseStack, buffer, packedLightIn, entity, 1.0f, 1.0f, 1.0f);
        }
