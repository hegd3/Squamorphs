...
public class SquamorphTailBaseLayer extends RenderLayer<SquamorphEntity, SquamorphModel> {


    public SquamorphTailBaseLayer(SquamorphRenderer pRenderer) {
        super(pRenderer);
    }
    ...
    public void render(PoseStack poseStack, MultiBufferSource buffer, int packedLightIn, SquamorphEntity entity, float v, float v1, float v2, float v3, float v4, float v5) {
        ...
        if (index != 4 && index != 0) {
            ...
            renderColoredCutoutModel(getParentModel(), AllTextures.TAIL_TEXTURES[0], poseStack, buffer, packedLightIn, entity, r, g, b);
        }
