...
public class SquamorphTailLayer extends RenderLayer<SquamorphEntity, SquamorphModel> {


    public SquamorphTailLayer(SquamorphRenderer pRenderer) {
        super(pRenderer);
    }
    ...
    public void render(PoseStack poseStack, MultiBufferSource buffer, int packedLightIn, SquamorphEntity entity, float v, float v1, float v2, float v3, float v4, float v5) {
        ...
        int index = entity.getTailType() - 1;
        if (index > 0) {
            int color = entity.getTail().getColor(entity);
            ...
            renderColoredCutoutModel(getParentModel(), AllTextures.TAIL_TEXTURES[index], poseStack, buffer, packedLightIn, entity, r, g, b);
        }
