...
public class SquamorphHeadLayer extends RenderLayer<SquamorphEntity, SquamorphModel> {


    public SquamorphHeadLayer(SquamorphRenderer pRenderer) {
    ...
    public void render(PoseStack poseStack, MultiBufferSource buffer, int packedLightIn, SquamorphEntity entity, float v, float v1, float v2, float v3, float v4, float v5) {
        ...
        int index = entity.getHeadType() - 1;
        if (index >= 0) {
            int color = entity.getHead().getColor(entity);
            ...
            renderColoredCutoutModel(getParentModel(), AllTextures.HEAD_TEXTURES[index], poseStack, buffer, packedLightIn, entity, r, g, b);
        }
